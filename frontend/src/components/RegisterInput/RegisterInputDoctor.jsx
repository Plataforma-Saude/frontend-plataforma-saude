...
export default function RegisterInputDoctor({
  label,
  type,
  name,
  placeholder,
  register,
  errors,
  mask, // 'crm', 'phone', 'zipcode' ou undefined
  setValue,
  isPassword = false,
  disabled,
  ...
  const handleChange = (e) => {
    ...
    if (mask === "crm") {
      ...
      // Máximo de 6 números no começo
      let numbers = val.match(/\d{0,6}/)?.[0] || "";
      // Máximo de 2 letras no final
      let letters = val.slice(numbers.length).replace(/[^a-zA-Z]/g, "").toUpperCase().slice(0, 2);

      // Adiciona hífen se tiver letras
      val = letters.length > 0 ? `${numbers}-${letters}` : numbers;
    }

    // (Aqui ficariam as outras máscaras: phone, zipcode etc.)
    setValue(name, val);
  };
